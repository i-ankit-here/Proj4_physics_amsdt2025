    ...
    return (
        <div className="bg-white container flex flex-col items-center lg:flex-row lg:justify-between lg:items-start max-w-7xl mx-auto px-5 sm:px-10 lg:px-8 py-20">
            <div className="w-full md:w-[700px] lg:w-3/5 px-4 lg:max-w-full">
                <h2 className="text-4xl font-sans font-bold text-[#2563eb] mb-4">
                    Sponsorship Rates
                </h2>
                ...
                <div className="bg-[white] border border-[#2563eb]/30 rounded-xl p-6 shadow-md hover:shadow-lg hover:shadow-[#2563eb]/20 transition-all duration-300">
                    <p className="text-base text-justify font-sans font-base text-gray-700 mb-6">
                    ...
                    <div className="overflow-x-auto">
                        <table className="mt-3 text-sm md:text-base border-collapse box-border min-w-full">
                            <thead>
                                <tr className="border-b-2 border-[#2563eb]/50">
                                    <th className="p-3 text-left text-[#2563eb]">Category</th>
                                    <th className="p-3 text-left text-[#2563eb]">Price in INR</th>
                                    <th className="p-3 text-left text-[#2563eb]">Category</th>
                                    <th className="p-3 text-left text-[#2563eb]">Price in INR</th>
                                </tr>
                            </thead>
                            <tbody>
                                {array.length > 0 ? array.map((item, index) => (
                                    <tr key={index} className="border-b border-[#2563eb]/20 hover:bg-[#2563eb]/10 transition-colors">
                                        <td className="p-3 text-left text-gray-700">{item.value1.category}</td>
                                        <td className="p-3 text-left text-[#2563eb]">{item.value1.price.toLocaleString()}</td>
                                        <td className="p-3 text-left text-gray-700">{item.value2?.category || ""}</td>
                                        <td className="p-3 text-left text-[#2563eb]">{item.value2?.price?.toLocaleString() || ""}</td>
                                    </tr>
                                )) : (
                                    <tr>
                                        <td colSpan="4" className="p-3 text-center text-gray-700">No data available</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            <div className="w-full md:w-[700px] px-4 lg:w-2/5 lg:max-w-[450px] mt-10 lg:mt-0 mx-auto lg:px-5 lg:border-l lg:border-[#2563eb]/30 lg:pl-8">
                <h2 className="text-4xl font-sans font-bold text-[#2563eb] mb-4">Souvenir</h2>
                <div className="w-24 h-1 bg-gradient-to-r from-[#2563eb] to-transparent mb-6"></div>
                ...
                <div className="bg-[white] border border-[#2563eb]/30 rounded-xl p-6 shadow-md hover:shadow-lg hover:shadow-[#2563eb]/20 transition-all duration-300 relative overflow-hidden">
                    {/* Decorative element */}
                    <div className="absolute top-2 right-2 w-3 h-3 rounded-full bg-[#2563eb]/50"></div>
                    <div className="absolute top-2 right-7 w-2 h-2 rounded-full bg-[#2563eb]/30"></div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="mt-3 border-collapse box-border min-w-full">
                            <thead>
                                <tr className="border-b-2 border-[#2563eb]/50">
                                    <th className="p-3 text-left text-[#2563eb]">Location</th>
                                    <th className="p-3 text-left text-[#2563eb]">Price in INR</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data2.length > 0 ? data2.map((item, index) => (
                                    <tr key={index} className="border-b border-[#2563eb]/20 hover:bg-[#2563eb]/10 transition-colors">
                                        <td className="p-3 text-left text-gray-700">{item.location}</td>
                                        <td className="p-3 text-left text-[#2563eb]">{item.price.toLocaleString()}</td>
                                    </tr>
                                )) : (
                                    <tr>
                                        <td colSpan="2" className="p-3 text-center text-gray-700">No data available</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
